...
describe 'Deployer' do
  ...
  describe '#push' do
    it "uploads file to S3 object when object doesn't exist" do
      ...
      uploader = Deployer::Deployer.new(config, s3_client: s3_object_does_not_exist, lambda_client: lambda_client)
      uploader.build
      uploader.push
      ...
      head_object_requests = s3_object_does_not_exist.api_requests.select { |x| x[:operation_name] == :head_object }
    ...
    it 'does not upload file to S3 object when object exists' do
      ...
      uploader = Deployer::Deployer.new(config, s3_client: s3_object_does_exist, lambda_client: lambda_client)
      uploader.push
